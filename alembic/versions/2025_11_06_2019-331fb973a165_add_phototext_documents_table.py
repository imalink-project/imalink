...
def upgrade() -> None:
    ...
    # Create basic indexes (compatible with both SQLite and PostgreSQL)
    ...
    op.create_index('ix_phototext_documents_created_at', 'phototext_documents', ['created_at'])
    op.create_index('ix_phototext_documents_visibility', 'phototext_documents', ['visibility'])


def downgrade() -> None:
    """Downgrade schema."""
    # Drop indexes
    op.drop_index('ix_phototext_documents_visibility', table_name='phototext_documents')
